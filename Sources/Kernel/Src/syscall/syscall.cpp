...
/* Sys_WaitPID :
    ...
    1 -> flags              > int
*/
KResult Sys_WaitPID(SyscallStack* Registers, kthread_t* Thread){
    if(CheckUserAddress((void*)Registers->arg1, sizeof(int)) != KSUCCESS) return KMEMORYVIOLATION;

    pid_t Pid = static_cast<pid_t>(Registers->arg0);
    int* Status = reinterpret_cast<int*>(Registers->arg1);
    int Flags = static_cast<int>(Registers->arg2);
    ...
    if(Pid <= 0){
        ...
        uint64_t Flags;
        if(Keyhole_Get(Thread, (key_t)Registers->arg0, DataTypeProcess, (uint64_t*)&ProcessFromPid, &Flags) != KSUCCESS) return KKEYVIOLATION;
