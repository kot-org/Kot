...
void kui_r_move_window(kui_Container *cnt, kui_Vec2 pos, kui_Vec2 offset){
  uint64_t PosX = (int)cnt->window_parent->window->Position.x + pos.x - offset.x;
  uint64_t PosY = (int)cnt->window_parent->window->Position.y + pos.y - offset.y;

  WindowChangePosition(cnt->window_parent->window, PosX, PosY);
}
