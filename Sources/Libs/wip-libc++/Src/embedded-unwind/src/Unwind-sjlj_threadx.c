...
struct _Unwind_FunctionContext
{
	...
	{
		return (struct _Unwind_FunctionContext*)tx_thread_getspecific(TX_THREAD_SJLJ_KEY);
	}

	static void __Unwind_SjLj_SetTopOfFunctionStack(struct _Unwind_FunctionContext* fc)
	{
		tx_thread_setspecific(TX_THREAD_SJLJ_KEY, fc);
	}
#endif
...
static _Unwind_Reason_Code unwind_phase1(struct _Unwind_Exception* exception_object)
{
	...
	debug_print("unwind_phase1: initial function-context=%p\n", c);
	...
	for (bool handlerNotFound = true; handlerNotFound; c = c->prev) {
		...
		if ( c == NULL ) {
			debug_print("unwind_phase1(ex_ojb=%p): reached bottom => _URC_END_OF_STACK\n", exception_object);
			return _URC_END_OF_STACK;
		}

		debug_print("unwind_phase1: function-context=%p\n", c);
		// if there is a personality routine, ask it if it will want to stop at this frame
		if ( c->personality != NULL ) {
			debug_print("unwind_phase1(ex_ojb=%p): calling personality function %p\n", exception_object, c->personality);
			...
			switch ( personalityResult ) {
				case _URC_HANDLER_FOUND:
					...
					debug_print("unwind_phase1(ex_ojb=%p): _URC_HANDLER_FOUND\n", exception_object);
					return _URC_NO_REASON;

				case _URC_CONTINUE_UNWIND:
					debug_print("unwind_phase1(ex_ojb=%p): _URC_CONTINUE_UNWIND\n", exception_object);
					// continue unwinding
					break;

				default:
					// something went wrong
					debug_print("unwind_phase1(ex_ojb=%p): _URC_FATAL_PHASE1_ERROR\n", exception_object);
...
{
	debug_print("unwind_phase2(ex_ojb=%p)\n", exception_object);

	// walk each frame until we reach where search phase said to stop
	_Unwind_FunctionContext_t c = __Unwind_SjLj_GetTopOfFunctionStack();
	while ( true ) {
		debug_print("unwind_phase2s(ex_ojb=%p): function-context=%p\n", exception_object, c);

		// check for no more frames
		if ( c == NULL ) {
			debug_print("unwind_phase2(ex_ojb=%p): unw_step() reached bottom => _URC_END_OF_STACK\n", exception_object);
			return _URC_END_OF_STACK;
		}
		...
		if ( c->personality != NULL ) {
			...
			switch ( personalityResult ) {
				case _URC_CONTINUE_UNWIND:
					// continue unwinding
					debug_print("unwind_phase2(ex_ojb=%p): _URC_CONTINUE_UNWIND\n", exception_object);
					if ( (uintptr_t)c == exception_object->private_2 ) {
						// phase 1 said we would stop at this frame, but we did not...
						panic("during phase1 personality function said it would stop here, but now if phase2 it did not stop here");
					}
					break;
				case _URC_INSTALL_CONTEXT:
					debug_print("unwind_phase2(ex_ojb=%p): _URC_INSTALL_CONTEXT, will resume at landing pad %p\n", exception_object, c->jbuf[1]);
				...
				default:
					// something went wrong
					debug_print("personality function returned unknown result %d\n", personalityResult);
					return _URC_FATAL_PHASE2_ERROR;
			}
		}
		c = c->prev;
												...
												_Unwind_Stop_Fn stop, void* stop_parameter)
{
	...
	while ( true ) {
		...
		debug_print("unwind_phase2_forced(ex_ojb=%p): stop function returned %d\n", exception_object, stopResult);
		if ( stopResult != _URC_NO_REASON ) {
			debug_print("unwind_phase2_forced(ex_ojb=%p): stopped by stop function\n", exception_object);
			return _URC_FATAL_PHASE2_ERROR;
		}
		...
		if ( c->personality != NULL ) {
			...
			debug_print("unwind_phase2_forced(ex_ojb=%p): calling personality function %p\n", exception_object, p);
			_Unwind_Reason_Code personalityResult = (*p)(1, action,
						exception_object->exception_class, exception_object,
						(struct _Unwind_Context*)c);
			switch ( personalityResult ) {
				case _URC_CONTINUE_UNWIND:
					debug_print("unwind_phase2_forced(ex_ojb=%p): personality returned _URC_CONTINUE_UNWIND\n", exception_object);
					// destructors called, continue unwinding
					break;
				case _URC_INSTALL_CONTEXT:
					debug_print("unwind_phase2_forced(ex_ojb=%p): personality returned _URC_INSTALL_CONTEXT\n", exception_object);
					// we may get control back if landing pad calls _Unwind_Resume()
					__Unwind_SjLj_SetTopOfFunctionStack(c);
					__builtin_longjmp(c->jbuf, 1);
					break;
				default:
					// something went wrong
					debug_print("unwind_phase2_forced(ex_ojb=%p): personality returned %d, _URC_FATAL_PHASE2_ERROR\n",
	...
	debug_print("unwind_phase2_forced(ex_ojb=%p): calling stop function with _UA_END_OF_STACK\n", exception_object);
...
{
	debug_print("_Unwind_SjLj_RaiseException(ex_obj=%p)\n", exception_object);
...
{
	debug_print("_Unwind_SjLj_Resume(ex_obj=%p)\n", exception_object);
	...
	panic("_Unwind_SjLj_Resume() can't return");
}
...
{
	debug_print("__Unwind_SjLj_Resume_or_Rethrow(ex_obj=%p), private_1=%ld\n", exception_object, exception_object->private_1);
	...
	panic("__Unwind_SjLj_Resume_or_Rethrow() called _Unwind_SjLj_Resume() which unexpectedly returned");
	return 0;
}
...
EXPORT uintptr_t _Unwind_GetLanguageSpecificData(struct _Unwind_Context* context)
{
	...
	debug_print("_Unwind_GetLanguageSpecificData(context=%p) => 0x%0lX\n", context, ufc->lsda);
	return ufc->lsda;
}
...
{
	debug_print("_Unwind_GetGR(context=%p, reg=%d)\n", context, index);
	_Unwind_FunctionContext_t ufc = (_Unwind_FunctionContext_t)context;
...
{
	debug_print("_Unwind_SetGR(context=%p, reg=%d, value=0x%0lX)\n", context, index, new_value);
	_Unwind_FunctionContext_t ufc = (_Unwind_FunctionContext_t)context;
...
{
	_Unwind_FunctionContext_t ufc = (_Unwind_FunctionContext_t)context;
	debug_print("_Unwind_GetIP(context=%p) => 0x%lX\n", context, ufc->resumeLocation+1);
...
EXPORT uintptr_t _Unwind_GetIPInfo(struct _Unwind_Context* context, int* ipBefore)
{
	_Unwind_FunctionContext_t ufc = (_Unwind_FunctionContext_t)context;
	*ipBefore = 0;
	debug_print("_Unwind_GetIPInfo(context=%p, %p) => 0x%lX\n", context, ipBefore, ufc->resumeLocation+1);
	return ufc->resumeLocation+1;
}
...
{
	debug_print("_Unwind_SetIP(context=%p, value=0x%0lX)\n", context, new_value);
	_Unwind_FunctionContext_t ufc = (_Unwind_FunctionContext_t)context;
...
EXPORT uintptr_t _Unwind_GetRegionStart(struct _Unwind_Context* context)
{
	...
	debug_print("_Unwind_GetRegionStart(context=%p)\n", context);
	return 0;
}
...
{
	debug_print("_Unwind_DeleteException(ex_obj=%p)\n", exception_object);
...
{
	// Not supported or needed for sjlj based unwinding
	debug_print("_Unwind_GetDataRelBase(context=%p)\n", context);
	panic("_Unwind_GetDataRelBase() not implemented");
	return 0;
}
...
{
	// Not supported or needed for sjlj based unwinding
	debug_print("_Unwind_GetTextRelBase(context=%p)\n", context);
	assert(0 && "_Unwind_GetTextRelBase() not implemented");
	return 0;
}
...
{
	debug_print("_Unwind_GetCFA(context=%p)\n", context);
	if ( context != NULL )
	{
		_Unwind_FunctionContext_t ufc = (_Unwind_FunctionContext_t)context;
