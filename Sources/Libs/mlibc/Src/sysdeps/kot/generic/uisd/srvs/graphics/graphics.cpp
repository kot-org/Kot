...
namespace Kot{
    kot_thread_t srv_graphics_callback_thread = NULL;
    uisd_graphics_t* GraphicsData = NULL;
    kot_process_t ShareProcessGraphics = NULL;

    void Srv_Graphics_Initialize(){
        GraphicsData = (uisd_graphics_t*)FindControllerUISD(ControllerTypeEnum_Graphics);
        kot_process_t Process = Sys_GetProcess();
        ShareProcessGraphics = ShareProcessKey(Process);

        kot_thread_t GraphicsThreadKeyCallback = NULL;
        Sys_CreateThread(Process, (uintptr_t)&Srv_Graphics_Callback, PriviledgeDriver, NULL, &GraphicsThreadKeyCallback);
        InitializeThread(GraphicsThreadKeyCallback);
    ...
    KResult Srv_Graphics_Createwindowcallback(KResult Status, struct srv_graphics_callback_t* Callback, uint64_t GP0, uint64_t GP1, uint64_t GP2, uint64_t GP3){
        if(Status == KSUCCESS){
            ...
            memcpy((void*)&Window->Framebuffer, (void*)GP1, sizeof(framebuffer_t));
    ...
    struct srv_graphics_callback_t* Srv_Graphics_ResizeWindow(window_t* Window, int64_t Width, int64_t Height, bool IsAwait){
        ...
        struct kot_arguments_t parameters;
        parameters.arg[0] = (uint64_t)srv_graphics_callback_thread;
        parameters.arg[1] = (uint64_t)callback;
