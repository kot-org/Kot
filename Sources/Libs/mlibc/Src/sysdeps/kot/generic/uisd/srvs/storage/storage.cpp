...
namespace Kot{
    ...
    struct srv_storage_callback_t* Srv_Storage_MountPartition(kot_thread_t VFSMountThread, struct srv_storage_fs_server_functions_t* FSServerFunctions, bool IsAwait){
        ...
        struct ShareDataWithArguments_t data;
        data.Data = (uintptr_t)FSServerFunctions;
    ...
    struct srv_storage_callback_t* Srv_Storage_Rename(char* OldPath, char* NewPath, bool IsAwait){
        ...
        struct srv_storage_fs_server_rename_t* RenameData = (struct srv_storage_fs_server_rename_t*)malloc(RenameDataSize);
        ...
        struct ShareDataWithArguments_t data;
        data.Data = (uintptr_t)RenameData;
    ...
    KResult Srv_Storage_DirOpen_Callback(KResult Status, struct srv_storage_callback_t* Callback, uint64_t GP0, uint64_t GP1, uint64_t GP2, uint64_t GP3){
        if(Status == KSUCCESS){
            ...
            Dir->DirProcessHandler = DirData->FSDriverProc;
            Callback->Data = (uint64_t)Dir;
            Callback->Size = sizeof(directory_t);
        }
        return Status;
    }
    ...
    KResult Srv_Storage_Readfile_Callback(KResult Status, struct srv_storage_callback_t* Callback, uint64_t GP0, uint64_t GP1, uint64_t GP2, uint64_t GP3){
        if(Status == KSUCCESS){
            uint64_t MemoryType;
            if(Sys_GetInfoMemoryField((kot_ksmem_t)GP0, &MemoryType, &Callback->Size) == KSUCCESS){
                if(MemoryType == MemoryFieldTypeSendSpaceRO){
                    return Sys_AcceptMemoryField(Sys_GetProcess(), (kot_ksmem_t)GP0, &Callback->Data);
                }
            }
        }
        return Status;
    ...
    KResult Srv_Storage_Writefile_Callback(KResult Status, struct srv_storage_callback_t* Callback, uint64_t GP0, uint64_t GP1, uint64_t GP2, uint64_t GP3){
        Sys_CloseMemoryField(Sys_GetProcess(), *(kot_ksmem_t*)Callback->Data, *(uintptr_t*)(Callback->Data + 8));
        return Status;
    }
