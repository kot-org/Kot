...
void Srv_Audio_Initialize(){
    ...
    kot_Sys_CreateThread(proc, &Srv_Audio_Callback, PriviledgeDriver, NULL, &AudioThreadKeyCallback);
...
KResult Srv_Audio_RequestStream_Callback(KResult Status, struct srv_audio_callback_t* Callback, uint64_t GP0, uint64_t GP1, uint64_t GP2, uint64_t GP3){
    if(Status == KSUCCESS){
        Callback->Data = malloc(sizeof(audio_share_buffer_t));
        memcpy(Callback->Data, GP0, sizeof(audio_share_buffer_t));
...
KResult Srv_Audio_GetDeviceInfo_Callback(KResult Status, struct srv_audio_callback_t* Callback, uint64_t GP0, uint64_t GP1, uint64_t GP2, uint64_t GP3){
    if(Status == KSUCCESS){
        Callback->Data = malloc(sizeof(srv_audio_device_info_t));
        memcpy(Callback->Data, GP0, sizeof(srv_audio_device_info_t));
