...
int create_exec(process_t* parent, int argc, char* args[], char* envp[]){
    ...
    void* stack;
    assert(!mm_allocate_region_vm(parent->memory_handler, NULL, PROCESS_STACK_SIZE, false, &stack));

    int err = load_elf_exec(process_ctx, argc, args, envp, stack);

    if(err){
        assert(!mm_free_region(parent->memory_handler, stack, PROCESS_STACK_SIZE));
