...
uint64_t Keyhole_Duplicate(thread_t* caller, key_t key, key_t* newKey, process_t* target, uint64_t flags){
    lock_t* data;
    if(Keyhole_Get(caller, key, DataTypeUnknow, &data) != KSUCCESS) return KFAIL;
...
uint64_t Keyhole_Verify(thread_t* caller, key_t key, enum DataType type){
    ...
    if(!vmm_GetFlags(lock->Parent->SharedPaging, (void*)VirtualAddress, vmm_flag::vmm_Custom0) || vmm_GetFlags(lock->Parent->SharedPaging, (void*)VirtualAddress, vmm_flag::vmm_PhysicalStorage) || !vmm_GetFlags(lock->Parent->SharedPaging, (void*)VirtualAddress, vmm_flag::vmm_Custom2)) return KFAIL;
